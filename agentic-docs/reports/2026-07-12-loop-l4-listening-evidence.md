...
L4 now contains the exact corrected L1–L3 dependency chain: zero/short/silent metric inputs and rejected alignment fail closed under `2026.07.12-l1.1`; runner subprocess/UTF-8/output-path failures are explicit; and metric `2026.07.12-l3.2` adds declared proximity/stiff-string/modal partial models plus attack/body/tail trajectories. The listening experiment digest remains `ed890e7bdfc129cbbbb8b3447665cffb47846d2c9cddbd9dc069681aaa55656e`, proving the metric migration did not silently rewrite the frozen hidden-reference/anchor protocol.

Exact-head local validation after the dependency merge: loop suite 55/55; listening Python suite 15/15; Chromium and desktop WebKit full A/B, ABX, MUSHRA, recovery, storage-failure, tampered-import, and browser-to-Python round trips; Rust 70/70; TypeScript typecheck/build; sustained 16-voice multi-track bench 74.9 µs/quantum (2.81% of the 2.67 ms budget); harness and diff checks. Physical iOS Safari remains external and is not inferred from desktop WebKit.
